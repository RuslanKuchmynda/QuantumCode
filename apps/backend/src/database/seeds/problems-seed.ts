...
export async function seedProblems() {
  for (const problem of seedProblemsData) {
    const [listResult] = await db
      .insert(problemListSchema)
      .values(problem.list)
      .returning();

    await db
      .insert(problemDetailsSchema)
      .values({
        id: listResult.id,
        example: JSON.stringify(problem.details.example),
        constraints: JSON.stringify(problem.details.constraints),
        starterCode: JSON.stringify(problem.details.starterCode),
